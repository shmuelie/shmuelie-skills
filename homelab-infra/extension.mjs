...
const session = await joinSession({
    hooks: {
        onUserPromptSubmitted: async (input) => {
            ...
            const triggers = [
                "proxmox", "pve", "lxc", "container passthrough",
                "nvidia gpu", "gpu passthrough", "pct exec", "pct push",
                "home assistant", "dashboard", "lovelace",
                "jellyfin plugin", "jellyfin provider", "imetadataprovider",
                "comfyui", "comfy ui", "custom node", "nodes_",
                "sd webui", "sdapi", "txt2img", "img2img",
                "qualcomm aic", "aic100",
                "proxmox-boot-tool", "update-initramfs",
            ];
    ...
    tools: [
        {
            name: "homelab_infra_guidance",
            description:
                "Get domain knowledge about homelab infrastructure: Proxmox GPU passthrough, LXC containers, Home Assistant dashboards, Jellyfin plugin development, ComfyUI custom nodes. Use for homelab/self-hosted service work.",
            parameters: {
                type: "object",
                properties: {
                    topic: {
                        type: "string",
                        description:
                            "The topic: 'proxmox-gpu', 'proxmox-boot', 'lxc', 'home-assistant', 'jellyfin', 'comfyui', or 'all'",
                        enum: ["proxmox-gpu", "proxmox-boot", "lxc", "home-assistant", "jellyfin", "comfyui", "all"],
                    },
                },
                required: ["topic"],
            },
            handler: async (args) => {
                ...
                const headings = {
                    "proxmox-gpu": "### NVIDIA GPU Passthrough",
                    "proxmox-boot": "### Proxmox Boot Management",
