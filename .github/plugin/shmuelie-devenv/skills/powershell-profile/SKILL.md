...
### Wrapping the copilot/agency CLIs (flag mapping)
- **`agency copilot` forwards unknown options to the engine as `EXTRA_ARGS`** (its usage is `agency copilot [OPTIONS] [EXTRA_ARGS]...`). So a mistyped or *renamed* agency-level flag does **not** error at the agency layer — it's passed through to the copilot engine, which usually doesn't know it either, so the intended behavior silently no-ops (or fails at the engine). Always validate agency flag names against the live `agency copilot --help`, not just "did it run".
- **Known rename:** the agency flag `--no-installed-plugins` became **`--no-config-plugins`** (the env var kept the old name, `AGENCY_NO_INSTALLED_PLUGINS`). A wrapper emitting the old flag is a silent no-op per the point above.
- **Agency stages `local:` plugins by their folder LEAF name, not the manifest name.** So two working-tree plugins with the same leaf folder (e.g. a plugin monorepo with `plugins/os/dev` **and** `plugins/shared/dev`, both leaf `dev`) can't both be loaded as `local:` — Agency fails plugin acquisition with *"Plugin folder name conflict for `dev` between local:…\os\dev and local:…\shared\dev"* even though their manifest `name`s differ (`os-dev` vs `dev`). Marketplace copies (`market:<name>@…`) don't hit this — Agency stages those by full plugin name (distinct). Consequence for the **local-plugin swap**: when it rewrites active `market:` specs to working-tree `local:` paths, it must de-collide by folder leaf — for any leaf shared by ≥2 active plugins, keep the marketplace copy for those (revert them out of the swap) and only swap the non-colliding ones. Note only the *active* set matters: `essentials` folders in three subdirs don't collide if only one `essentials` is actually active.
- **Verify arg-mapping with `-WhatIf`, not by launching.** Give the wrapper `[CmdletBinding(SupportsShouldProcess)]` and gate the exec on `$PSCmdlet.ShouldProcess("$exe $displayArgs", 'Execute')`. Then `Start-Copilot -NoResume <flags> -WhatIf` renders the *full* command line (`What if: … on target "<exe> copilot <args>"`) without spawning a session — the fast, side-effect-free way to confirm a flag maps to the right CLI option (and to diff agency vs bare-copilot arg arrays).
- **Agency built-in MCP mapping** (`$agencyMcpEquivalents`): under agency, prefer Agency's built-in MCP over the JSON-config equivalent — `--disable-mcp-server <JsonName>` + `--mcp <agency-name>`. Keep the `--mcp` add unconditional (always use agency's version) but **guard the `--disable-mcp-server` side by presence** in `~/.copilot/mcp-config.json`, so you don't emit dead disable flags for servers the active config doesn't contain. The map is a union over the work and consumer MCP configs.

### Restoring the terminal after an engine crash
- A TUI engine (copilot/agency) that exits non-zero may have crashed mid-frame and left the terminal in a bad state: mouse tracking, focus reporting, the alternate screen buffer, bracketed paste, synchronized output, and the kitty keyboard protocol all still enabled. After the exec, when `$LASTEXITCODE -ne 0` (and stdout isn't redirected — guard with `[Console]::IsOutputRedirected` so the bytes can't corrupt captured `-p` output), emit a reset so the shell is usable again.
...
- **Two independent "pick exactly one" axes need a cross-product of parameter sets.** PowerShell resolves a call to exactly **one** parameter set, so you can't have two independent set axes (e.g. an *engine* axis {Copilot, Agency} and a *resume-mode* axis {default, NoResume, ResumeLatest, ResumeSession, ShowPicker}). Enumerate the **matrix** of named sets (`Copilot`, `CopilotNoResume`, `AgencyNoResume`, `AgencyResumeSession`, …), tag each axis-A param into all of its A-flavored sets and each axis-B param into all of its B-flavored sets, and leave shared params **untagged** (they land in every set). This is how `Start-Copilot` keeps `-Agency` (and its 23 sub-params) combinable with the resume switches while still enforcing "one resume mode at a time" and "sub-params require `-Agency`".
- **A set-defining switch must be `Mandatory` in its own sets, or it silently swallows the default set.** If `-Agency` is `Mandatory` across the 5 `Agency*` sets but the resume switches are *optional* in theirs, then `-Agency` alone satisfies **all 5** Agency sets equally → *"Parameter set cannot be resolved"* (and `DefaultParameterSetName` can't rescue it because the default `Copilot` set isn't a candidate once `-Agency` is bound). Fix: make **each** set-defining switch (`-NoResume`, `-ResumeSession`, `-ShowPicker`, …) `Mandatory` in its own sets, so a set only becomes a candidate when its switch is actually present, and the base engine set (`Agency`) resolves uniquely from `-Agency` alone. Verify the whole matrix with `-WhatIf` (bare, each engine, each combo, and every conflict should error).
...
- **Renaming a set-defining switch: rename the *parameter*, keep the *set name*.** To rename a user-facing switch while preserving back-compat, add an `[Alias('OldName')]` and rename only the parameter variable (e.g. `-ShowPicker` → `[Alias('ShowPicker')][switch]$NoAutoResume`). A parameter alias is honored in binding **and** parameter-set resolution, so leave the internal `ParameterSetName` strings (e.g. `CopilotShowPicker`/`AgencyShowPicker`) and any `ParameterSetName -like '*ShowPicker'` checks unchanged — `$PSCmdlet.ParameterSetName` returns the same set for both the new name and the alias. This avoids re-tagging every param in the set (the `Agency*` sub-params) and keeps the diff to the one declaration + help.
