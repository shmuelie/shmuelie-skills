...
const session = await joinSession({
    hooks: {
        onUserPromptSubmitted: async (input) => {
            ...
            const triggers = [
                "winui", "msix", "winappsd", "winappsdk", "win ui",
                "x:bind", "xbind", "{binding", "bindable",
                "appxmanifest", "package.appxmanifest",
                "enablemsix", "windowspackagetype",
                "add-appxpackage", "winappdeploycmd",
                "datatemplate", "itemsrepeater", "gridview",
    ...
    tools: [
        {
            name: "winui3_msix_guidance",
            description:
                "Get domain knowledge about WinUI 3 binding patterns, MSIX packaging, WinAppSDK test architecture, and deployment. Use when working on WinUI 3 / WinAppSDK / MSIX projects.",
            parameters: {
                type: "object",
                properties: {
                    topic: {
                        type: "string",
                        description:
                            "The topic to get guidance on: 'binding', 'msix', 'testing', 'deployment', 'layout', or 'all'",
                        enum: ["binding", "msix", "testing", "deployment", "layout", "all"],
                    },
                },
                required: ["topic"],
            },
            handler: async (args) => {
                ...
                const sections = {
                    binding: SKILL_KNOWLEDGE.split("## MSIX")[0],
                    msix: SKILL_KNOWLEDGE.split("## MSIX Packaging")[1]?.split("## Test Architecture")[0] || "",
                    testing: SKILL_KNOWLEDGE.split("## Test Architecture")[1]?.split("## DI Pattern")[0] || "",
                    deployment: SKILL_KNOWLEDGE.split("## MSIX Deployment")[1] || "",
                    layout: SKILL_KNOWLEDGE.split("### Layout Gotchas")[1]?.split("## MSIX")[0] || "",
                };
