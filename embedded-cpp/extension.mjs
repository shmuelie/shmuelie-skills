...
const session = await joinSession({
    hooks: {
        onUserPromptSubmitted: async (input) => {
            ...
            const triggers = [
                "cmake", "buildroot", "cross-compil", "mips",
                "fetchcontent", "catch2", "upx",
                "embedded", "iot", "mfi",
                "-fno-rtti", "-fno-unwind", "binary size",
                "cfgmtd", "mosquitto", "mqtt auto-discovery",
                "snake_case", "c++20", "c++ embedded",
            ];
    ...
    tools: [
        {
            name: "embedded_cpp_guidance",
            description:
                "Get domain knowledge about embedded C++/CMake: Buildroot cross-compilation, binary size optimization, FetchContent, Catch2 testing, C++20 patterns, MQTT/HA integration. Use for embedded Linux or IoT C++ projects.",
            parameters: {
                type: "object",
                properties: {
                    topic: {
                        type: "string",
                        description:
                            "The topic: 'buildroot', 'size-optimization', 'cmake-deps', 'catch2', 'conventions', 'deployment', 'mqtt', or 'all'",
                        enum: ["buildroot", "size-optimization", "cmake-deps", "catch2", "conventions", "deployment", "mqtt", "all"],
                    },
                },
                required: ["topic"],
            },
            handler: async (args) => {
                ...
                const headings = {
                    buildroot: "## Buildroot",
                    "size-optimization": "## Binary Size",
                    "cmake-deps": "## CMake Dependency",
                    catch2: "### Catch2",
                    conventions: "## C++20 Embedded",
                    deployment: "## Embedded Device Deployment",
